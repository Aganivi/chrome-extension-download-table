...
if (table) {
	for (let i = 0; i < table.length; i++) {
		const startNode = table[i]?.parentNode
		console.log(`startNode:`, startNode)
		startNode.classList.add('color-secondary-text', 'type--caption')
	}
	...
	// badge.textContent = `⏱️  min read`
	// console.log(`badge.textContent:`, badge.textContent)

	// // Support for API reference docs
	// const heading = document.querySelector('mytable')
	// if (heading) heading.classList.add('color-secondary-text', 'type--caption')
	// // Support for article docs with date
	// const date = article.querySelector('thead')?.parentNode
